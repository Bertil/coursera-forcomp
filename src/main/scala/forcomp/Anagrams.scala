...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = s match {
    case Nil => Nil
    case s => wordOccurrences(s.reduce(_ + _))
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) List(List())
      ...
      (for {
        combo <- combinations(occurrences.tail)
        n <- 1 to occurrences.head._2
      } yield (occurrences.head._1, n) :: combo) ++ combinations(occurrences.tail)
    }
  }
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x.map(a=>(a._1, a._2 - (y.indexWhere(_._1 == a._1) match {
    case -1 => 0
    case n => y(n)._2
  }) )).filter(_._2 > 0)
